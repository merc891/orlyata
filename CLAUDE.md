...
— Используй скилл frontend-design и lazyweb skill для создания дизайна новых страниц.
...
- Менять `line-height`/`letter-spacing` в media-query — на всех брейкпоинтах один элемент использует **один текстовый стиль**, меняется только `font-size` (см. `directives/responsive.md`)
- Ломать типографическую иерархию в адаптиве — если A крупнее B на десктопе, A должен оставаться крупнее B на каждом брейкпоинте (см. `directives/responsive.md`)
